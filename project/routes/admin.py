...
@app.route("/all_agents",method=['GET'])
@token_required
...
#modefied
@app.route("/all_Agent_applications",method=['GET'])
@token_required
def get_all_agents(loggedInUser):
    if loggedInUser.user_type==USERTYPE['Agent'] or loggedInUser.user_type == USERTYPE['Customers']:
        return jsonify({"Message":"You are not authorized to open this page"})
    ...
    if len(response["Unapproved_agents"])==0:
        return jsonify({"Message":"No pending applications agents in the system!"})
    return jsonify(response)
...
def approve_agent_loan(loggedInUser,agent_id):
    ...
    if len(response["Agent_loanRequests"])==0:
        return jsonify({"Message":"No request for loans from this particular agent found in system"})
    return jsonify(response)
...
def approve_agent(loggedInUser,agent_id):
    ...
    agent.approved=True
    db.session.commit()
    return jsonify({"meassage":"the agent has been sucessfully approved!"})
